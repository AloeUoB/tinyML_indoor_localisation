...
flags.DEFINE_string("source_model_dir", "model_path/trained_LSTM_model_house_C.h5",
                    "the directory where the trained model can be found.")
flags.DEFINE_string("target_dir", "model_path/",
                    "the directory to save the quant model.")
flags.DEFINE_string("model_name", "LSTM_int8",
                    "name to save the quant model.")
...
def get_data():
  ...
  x_values = np.random.uniform(low=0, high=1,
                               size=(200, 20, 11)).astype(np.float32)
...
def convert_quantized_tflite_model(source_model_dir, x_values):
  ...
  def representative_dataset(num_samples=len(x_values)):
    for i in range(num_samples):
      yield [x_values[i].reshape(1, 20, 11)]

  converter = tf.lite.TFLiteConverter.from_keras_model(source_model_dir)
